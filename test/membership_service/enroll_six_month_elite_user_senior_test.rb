...
class EnrollSixMonthEliteSeniorTest < Common
  def setup
    super
  ...
  def select_six_month_elite
    @browser.element('data-offering-id': '3', 'data-payment-plan-id': '2').click
  end

  def check_membership_cost
    @membership_cost = @browser.elements(class: 'pricing-information-row__text__digits--small-digits')[1].text.to_i
  end
